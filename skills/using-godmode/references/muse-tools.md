...
Muse has native skill support via `muse skills`. To invoke a Superpowers skill, read its `SKILL.md` and follow the instructions. The bootstrap (`using-godmode`) is injected automatically at `SessionStart` via the plugin hook — you are already following it, do not re-load it.
