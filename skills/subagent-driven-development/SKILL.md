...
- Each plan owns a workspace: at skill start, run this skill's
  ...
  directory (under `<repo-root>/.godmode/sdd/`), home to
...
- Check for this plan's ledger at `<workspace>/progress.md`. If its first
  ...
  ledger at the old flat path `.godmode/sdd/progress.md` — is another
...
**NEEDS_CONTEXT:** The implementer needs information that wasn't provided. Provide the missing context and re-dispatch. If the missing piece is a fact about a library, API, or tool version rather than a decision, get it with godmode:research (a background agent against primary sources) and hand the implementer the research file path. If the missing piece is a seam the brief's "Seam under test" cannot reach, that is a plan defect: rule on the seam using godmode:codebase-design vocabulary, ledger it, and re-dispatch.
...
- **Reviewer inputs:** the task reviewer gets three paths — the same brief
  ...
  constraints that bind the task, the repo's standards files, and the
...
- **Two axes, one seat.** The per-task reviewer returns two independent
  ...
  sub-agents. A spec ❌ is never offset by a clean Standards verdict, and
  the reverse. Minor smells and graded refactoring notes go to the ledger
  as deferred minors: refactoring happens in review, not in the
  implementer's RED-GREEN loop.
...
**REQUIRED SUB-SKILL:** run godmode:requesting-code-review's two-axis
review: a Standards sub-agent and a Spec sub-agent, dispatched in
parallel, both on the most capable available model (see Model
Selection), both built on
...
smell in more than one task (Shotgun Surgery, Divergent Change, Middle
Man, a seam no test could reach), add one line under "Architecture
follow-up" naming the area and recommending godmode:improve-codebase-architecture
for it. Recommend it; do not start it.
