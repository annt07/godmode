...
description: Use when a godmode session went wrong and your human partner wants to know why — repeated work, ignored plans, stumbles, poor results, a skill that didn't fire, "it took too long", "why is it so expensive", "what is it doing" — or wants to build a bug report about the godmode skill set, for the current session or a past one identified by id or path, on any harness.
---
...
you do not fix godmode. Whoever triages the bundle or the issue
decides whether godmode changes.
...
2. **Locate.** Resolve each session to verified absolute filesystem paths using
   ...
   `~/.godmode/diagnosing-godmode/<session-id>/`, tell your
